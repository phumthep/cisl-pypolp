...
def convert_df_to_problems(
    ...
    return (
        ...
        DWProblem(c_df, A_df, b_df, ineq_df, col_df, row_indices, col_indices, n_subproblems, master_size)
        )
...
def get_dataframes(
    ...
    ineq_df = pd.DataFrame(['L']*A.shape[0], index=row_names, columns=['value'])
