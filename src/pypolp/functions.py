...
def get_pypolp_dir() -> str:
    local_dir = os.getcwd()
    return os.path.dirname(local_dir)
...
def get_non_binary_from_df(
    ...
    target_variables["variable"] = (
        target_variables["name"].str.split("[", expand=True).iloc[:, 0]
    )
    target_variables = target_variables.loc[
        target_variables["variable"].isin(target_varnames), :
    ]
...
def separate_master_vars(master_vars: pd.DataFrame) -> tuple[pd.DataFrame]:
    ...
    """
    master_only_vars = master_vars[
        ~master_vars["variable"].str.contains("B\(", regex=True)
    ]
