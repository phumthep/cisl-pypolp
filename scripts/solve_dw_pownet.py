...
def run_dw_experiment(
    ...
    session_name = f"{ctime}_{model_name}_{T_simulate}_{set_rmpgap}_{set_dwimprove}_{num_threads}"
    ...
    num_instances = 5  # count_mps_files(instance_folder)
    for k in range(num_instances):
        ...
        if not dw_is_int:
            ...
            non_bin_variables.to_csv(os.path.join(
                output_dir, fname), index=False)
            raise ValueError(
        ...
        mip_model.setParam("Threads", num_threads)
