...
class GurobipyOptimizer:
    ...
    def __init__(
        ...
        self.status_map = {2: 'optimal', 3: 'infeasible',
        ...
        if to_relax and get_dw_verbose():
            print(
                f'DW Solve: Relax {self.model.modelname} to a linear program.')
            self.model.relax()
    ...
    def get_gp_model_from_dataframes(
        ...
        model_constrs = model.addMConstr(
            A.values, model_vars, inequalities['value'].values, rhs.values.reshape(
                -1)
        )
